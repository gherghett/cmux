...
- [ ] **Dtach lifetime management overhaul** — current reconciliation only scans socket FILES, but dtach processes can survive after their socket is deleted (SIGTERM ignored, race condition in killDtachBySocket). Need to also scan /proc for dtach processes referencing our runtime dir. Bigger picture: consider a dtach registry (pid file, or a simple manifest) so we always know what's running. Related ideas: make it easy to manually reattach a dtach session (e.g. `cmux-cli attach <id>`), list all dtach sessions with their status (`cmux-cli list-sessions`), adopt orphaned dtach into a workspace. Might also evaluate alternatives to dtach (abduco has screen redraw, or a custom fork manager).
